...
local function LoadSettings()
    start_offset = tonumber(reaper.GetExtState("JKK_Item Tool", "start_offset")) or 1.0
    width = tonumber(reaper.GetExtState("JKK_Item Tool", "width")) or 5.0
    pos_range = tonumber(reaper.GetExtState("JKK_Item Tool", "pos_range")) or 0.0
    pitch_range = tonumber(reaper.GetExtState("JKK_Item Tool", "pitch_range")) or 0.0
    playback_range = tonumber(reaper.GetExtState("JKK_Item Tool", "playback_range")) or 0.0
    vol_range = tonumber(reaper.GetExtState("JKK_Item Tool", "vol_range")) or 0.0
    ...
    random_pos   = LoadFlag("JKK_Item Tool", "random_pos", true)
    random_pitch = LoadFlag("JKK_Item Tool", "random_pitch", true)
    random_play  = LoadFlag("JKK_ItemTooler", "random_play", true)
    random_vol   = LoadFlag("JKK_ItemTooler", "random_vol", true)
    random_order = LoadFlag("JKK_ItemTooler", "random_order", false)
    live_update  = LoadFlag("JKK_ItemTooler", "live_update", true)

    update_prev() 
end

local function SaveSettings()
    reaper.SetExtState("JKK_ItemTooler", "start_offset", tostring(start_offset), true)
    reaper.SetExtState("JKK_ItemTooler", "width", tostring(width), true)
    reaper.SetExtState("JKK_ItemTooler", "pos_range", tostring(pos_range), true)
    reaper.SetExtState("JKK_ItemTooler", "pitch_range", tostring(pitch_range), true)
    reaper.SetExtState("JKK_ItemTooler", "playback_range", tostring(playback_range), true)
    reaper.SetExtState("JKK_ItemTooler", "vol_range", tostring(vol_range), true)
    
    reaper.SetExtState("JKK_ItemTooler", "random_pos", random_pos and "1" or "0", true)
    reaper.SetExtState("JKK_ItemTooler", "random_pitch", random_pitch and "1" or "0", true)
    reaper.SetExtState("JKK_ItemTooler", "random_play", random_play and "1" or "0", true)
    reaper.SetExtState("JKK_ItemTooler", "random_vol", random_vol and "1" or "0", true)
    reaper.SetExtState("JKK_ItemTooler", "random_order", random_order and "1" or "0", true)
    reaper.SetExtState("JKK_ItemTooler", "live_update", live_update and "1" or "0", true)
end
...
function main()
    ...
    local visible, open_flag = reaper.ImGui_Begin(ctx, 'JKK_ItemTooler', open,
    ...
    if visible then
        ...
        if changed_group_stretch then
            ...
            if group_stretch_ratio ~= prev_group_stretch_ratio then
                ...
                prev_project_state_count = reaper.GetProjectStateChangeCount(0)
            end
        end
        ...
        if current_project_state_count ~= prev_project_state_count and not is_group_stretch_slider_active then
            ...
            local needs_reset = group_stretch_ratio ~= 1.0 or prev_group_stretch_ratio ~= 1.0
        ...
        if reaper.ImGui_Button(ctx, 'Move to Edit Crsr', 208, 22) then
            MoveItemsToEditCursor()
        end
        reaper.ImGui_SameLine(ctx)

        if reaper.ImGui_Button(ctx, 'Render to Stereo', 208, 22) then
