...
## 2. 소개하기
1. **JKK_Item Manager**
    - Item Batch Controller
    - Item Arranger & Randomizer
    - Region Creator (from Selected Items)
        - _This feature exists as a separate script._
3. **JKK_Track Manager**
    - Track Batch Controller (Vol & Pan)
    - Track Selector (by Folder Level)
    - Time Selection Creator (from Selected Track Items)
        - _This feature exists as a separate script._
    - Region Creator (from Selected Track Items)
        - _This feature exists as a separate script._
    - Unused Tracks Remover
        - _This feature exists as a separate script._
4. **JKK_Resion Manager**
    - JJK_FastFileRenamer - Batch File Renamer Tool
      region 이름 한번에 수정하기 Fast File Renamer같은거
    - JJK_Auto Increment Items
    - *mc_Create Region from Selected Items*
    - 타임 셀렉션에 걸쳐있는 region만 한번에 선택
    - 영역 색깔 한번에 변경
